...
class WallpaperManager(object):
	...
	def __is_image(self, filename) -> bool:
		if path.isfile(path.abspath(path.join(self.directory, filename))) and path.splitext(filename)[1] in img_formats:
			return True
		else:
			return False
	...
	def __load_wallpaper_list(self):
		...
		for i in files:
			if self.__is_image(i):
				#self.wp_list.append(path.abspath(path.join(self.directory, i)))
				self.wp_list.append(Wallpaper(path.abspath(path.join(self.directory, i)), self.__get_wallpaper_weight(path.abspath(path.join(self.directory, i))), self.__get_wallpaper_size(path.abspath(path.join(self.directory, i)))))
			else:
